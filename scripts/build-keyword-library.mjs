...
const sourcePath = path.resolve("docs/keyword-library/chopsticks-keyword-library.csv");
...
for (const group of groups) {
  ...
  markdown.push("|---|---:|---|---|---|---|");
  for (const row of group.rows.slice(0, 40)) {
...
fs.writeFileSync(path.join(outDir, "chopsticks-keyword-library.md"), markdown.join("\n"), "utf8");
console.log(`Wrote ${path.join(outDir, "chopsticks-keyword-library.md")}`);

function groupTopKeywords(rows) {
  const definitions = [
    { name: "Tutorial", test: (row) => row.category === "tutorial" },
    { name: "Materials and Types", test: (row) => row.category === "types-products" },
    { name: "Etiquette and Culture", test: (row) => row.category === "culture-etiquette" },
    { name: "General Expansion", test: () => true }
  ];
